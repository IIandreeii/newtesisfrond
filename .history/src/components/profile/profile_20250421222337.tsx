...
export default function UserProfile() {
  ...
  // Determinar el tipo de usuario basado en la respuesta de la API
  useEffect(() => {
    if (getResponse) {
      // Verificar si es un usuario o una organización según la estructura de datos
      // Esto dependerá de la estructura exacta de tu API, ajusta según sea necesario
      if (getResponse.type === "organization" || getResponse.userType === "organization") {
  ...
  // Prepara la estructura de datos del perfil según el tipo y los datos de la API
  const profileData = useMemo(() => {
    if (!getResponse) {
      // Si no hay respuesta de la API, usar los datos de ejemplo según el tipo
      return userType === "user" ? userData : organizationData;
    }

    // Si hay datos de la API, construir el objeto de perfil
    if (userType === "user") {
      return {
        id: getResponse.id || userData.id,
        email: getResponse.email || userData.email,
        dni: getResponse.dni || userData.dni,
        apellido: getResponse.apellido || userData.apellido,
        nombre: getResponse.nombre || userData.nombre,
        role: getResponse.role || userData.role,
        userType: "user",
        memberSince: getResponse.createdAt ? new Date(getResponse.createdAt).toLocaleDateString('es-ES', { year: 'numeric', month: 'long' }) : userData.memberSince,
        avatar: getResponse.avatar || userData.avatar,
        stats: {
          totalDonations: getResponse.stats?.totalDonations || userData.stats.totalDonations,
          totalAmount: getResponse.stats?.totalAmount || userData.stats.totalAmount,
          productDonations: getResponse.stats?.productDonations || userData.stats.productDonations,
          impactPoints: getResponse.stats?.impactPoints || userData.stats.impactPoints,
        }
      };
    } else {
      return {
        id: getResponse.id || organizationData.id,
        nombre: getResponse.nombre || organizationData.nombre,AZZZZ
        descripcion: getResponse.descripcion || organizationData.descripcion,
        email: getResponse.email || organizationData.email,
        direccion: getResponse.direccion || organizationData.direccion,
        telefono: getResponse.telefono || organizationData.telefono,
        userType: "organization",
        memberSince: getResponse.createdAt ? new Date(getResponse.createdAt).toLocaleDateString('es-ES', { year: 'numeric', month: 'long' }) : organizationData.memberSince,
        avatar: getResponse.avatar || organizationData.avatar,
        stats: {
          totalProjects: getResponse.stats?.totalProjects || organizationData.stats.totalProjects,
          totalDonationsReceived: getResponse.stats?.totalDonationsReceived || organizationData.stats.totalDonationsReceived,
          totalBeneficiaries: getResponse.stats?.totalBeneficiaries || organizationData.stats.totalBeneficiaries,
          activeProjects: getResponse.stats?.activeProjects || organizationData.stats.activeProjects,
        }
      };
    }
  }, [getResponse, userType, userData, organizationData]);
              ...
              {userType === "user" ? (
                <>
                  <div className="bg-slate-50 p-3 rounded-lg text-center">
                    <p className="text-xl font-bold text-slate-800">{profileData.stats.totalDonations}</p>
                    <p className="text-xs text-slate-500">Donaciones</p>
                  </div>
                  <div className="bg-slate-50 p-3 rounded-lg text-center">
                    <p className="text-xl font-bold text-slate-800">{profileData.stats.totalAmount}</p>
                    <p className="text-xs text-slate-500">Total donado</p>
                  </div>
                  <div className="bg-slate-50 p-3 rounded-lg text-center">
                    <p className="text-xl font-bold text-slate-800">{profileData.stats.productDonations}</p>
                    <p className="text-xs text-slate-500">Productos</p>
                  </div>
                  <div className="bg-slate-50 p-3 rounded-lg text-center">
                    <p className="text-xl font-bold text-slate-800">{profileData.stats.impactPoints}</p>
                ...
                <>
                  <div className="bg-slate-50 p-3 rounded-lg text-center">
                    <p className="text-xl font-bold text-slate-800">{profileData.stats.totalProjects}</p>
                    <p className="text-xs text-slate-500">Proyectos</p>
                  </div>
                  <div className="bg-slate-50 p-3 rounded-lg text-center">
                    <p className="text-xl font-bold text-slate-800">{profileData.stats.totalDonationsReceived}</p>
                    <p className="text-xs text-slate-500">Donaciones recibidas</p>
                  </div>
                  <div className="bg-slate-50 p-3 rounded-lg text-center">
                    <p className="text-xl font-bold text-slate-800">{profileData.stats.totalBeneficiaries}</p>
                    <p className="text-xs text-slate-500">Beneficiarios</p>
                  </div>
                  <div className="bg-slate-50 p-3 rounded-lg text-center">
                    <p className="text-xl font-bold text-slate-800">{profileData.stats.activeProjects}</p>
