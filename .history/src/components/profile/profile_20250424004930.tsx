...
import type { UserProfile, CharityProfile } from "./service/profile"

// Funciones auxiliares para verificar el tipo
const isDonor = (profile: UserProfile | CharityProfile): profile is UserProfile => {
  return (profile as UserProfile).dni !== undefined;
}

const isCharity = (profile: UserProfile | CharityProfile): profile is CharityProfile => {
  return (profile as CharityProfile).descripcion !== undefined;
}

export default function UserProfile() {
  const [userType, setUserType] = useState<"donor" | "charity">("donor")
  const [profileData, setProfileData] = useState<UserProfile | CharityProfile | null>(null)
  ...
  useEffect(() => {
    if (getResponse) {
      console.log("Respuesta completa:", getResponse);
      
      // Extraer los datos según la estructura real de la respuesta
      let userData = null;
      
      // Caso 1: Los datos están en getResponse.data.user o getResponse.data.charity
      if (getResponse.data) {
        console.log("Datos recibidos en data:", getResponse.data);
        
        userData = 'charity' in getResponse.data 
          ? getResponse.data.charity 
          : 'user' in getResponse.data 
            ? getResponse.data.user 
            : null;
      } 
      // Caso 2: Los datos están directamente en la raíz de getResponse
      else if ('charity' in getResponse) {
        console.log("Datos recibidos en la raíz (charity):", getResponse.charity);
        userData = getResponse.charity;
      } 
      else if ('user' in getResponse) {
        console.log("Datos recibidos en la raíz (user):", getResponse.user);
        userData = getResponse.user;
      }
      
      console.log("Datos extraídos:", userData);
      
      if (userData) {
        if ('userType' in userData) {
          setUserType(userData.userType as "donor" | "charity");
        } else if (isDonor(userData)) {
          setUserType("donor");
        } else if (isCharity(userData)) {
          setUserType("charity");
        }
        
        setProfileData(userData);
      } else {
        console.error("No se pudieron extraer datos de usuario de la respuesta");
      }
    } else {
      console.log("getResponse es null o undefined");
    }
  }, [getResponse]);
  ...
  // Aquí puedes renderizar el contenido específico según el tipo
  const renderUserInfo = () => {
    if (isDonor(profileData)) {
      return (
        <>
