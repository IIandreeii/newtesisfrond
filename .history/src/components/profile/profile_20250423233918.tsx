...
function ProfileNavButton({
  ...
  // Lista de rutas que aún no están implementadas
  const rutasNoImplementadas = [
    '/perfil/donaciones-recibidas',
    '/perfil/proyectos',
    '/perfil/auditorias',
    '/perfil/donaciones',
    '/perfil/mensajes',
  ...
  const estaImplementada = !rutasNoImplementadas.includes(href);
  
  const cardContent = (
    <Card className={`border-slate-100 shadow-sm transition-all duration-300 ${estaImplementada ? 'hover:border-slate-200 hover:shadow-md hover:-translate-y-1' : 'opacity-70'} h-full`}>
  ...
  // Si la ruta no está implementada, renderizamos sin Link
  if (!estaImplementada) {
    return cardContent;
  }
  
  // Si la ruta está implementada, usamos Link
...
function LogoutButton() {
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      router.push("/logind"); // Redirige al usuario a la página de login
    } catch (error) {
  ...
  return (
    <Button
      ...
      disabled={isLoggingOut || loader.loading}
    >
      {isLoggingOut || loader.loading ? (
        <div className="h-4 w-4 animate-spin rounded-full border-t-2 border-slate-600"></div>
      ) : (
