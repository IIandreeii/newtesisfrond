...
export default function UserProfile() {
  ...
  const [userType, setUserType] = useState<"user" | "organization">("user") // Por defecto mostramos la vista de usuario

  // Simulación de datos de usuario/organización
          ...
          <div className="flex-1 text-center md:text-left">
            <div className="space-y-1">
              <h1 className="text-2xl font-bold text-slate-800">
                {userType === "user" ? `${profileData.nombre} ${profileData.apellido}` : profileData.nombre}
              </h1>
              <p className="text-slate-500">{profileData.email}</p>
              {userType === "organization" && (
                <p className="text-slate-500">
                  {userType === "organization" && `${profileData.direccion} • ${profileData.telefono}`}
                </p>
              )}
