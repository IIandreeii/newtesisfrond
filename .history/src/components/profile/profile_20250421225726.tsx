...
import { profileService } from "./service/profile" // Asumiendo que crearás este archivo
...
export default function UserProfile() {
  ...
  // Utilizamos el servicio de perfil
  const { getProfile, loader, getResponse } = profileService()
  
  // Datos por defecto que se mostrarán mientras carga o si hay un error
  const donorData = {
  ...
  // Efecto para actualizar los datos cuando se recibe la respuesta de la API
  useEffect(() => {
    if (getResponse) {
      // Actualizamos el tipo de usuario según la respuesta
      if (getResponse.userType) {
        setUserType(getResponse.userType);
      }
      
      // Actualizamos los datos del perfil
      setProfileData(getResponse);
    }
  }, [getResponse]);
  ...
  useEffect(() => {
    ...
    if (authToken) {
      ...
      setLoading(false); // No está cargando si no hay token
      // Opcionalmente redirigir al login
    }
  }, [authToken]);
  

  // Determinamos qué datos mostrar según el tipo de usuario y si tenemos datos de la API
  const userData = profileData || (userType === "donor" ? donorData : charityData);
            ...
            <div className="relative">
              <div className="w-24 h-24 rounded-full overflow-hidden border-4 border-white shadow-md">
                <Image
                  ...
                  alt={userType === "donor" ? userData.nombre : userData.nombre}
                  width={120}
                  height={120}
            ...
            <div className="space-y-1">
              {userType === "donor" ? (
                <>
                  ...
                  <p className="text-slate-500">DNI: {userData.dni}</p>
                </>
              ) : (
                <>
                  <h1 className="text-2xl font-bold text-slate-800">{userData.nombre}</h1>
                  <p className="text-slate-600 italic">{userData.descripcion}</p>
                  <p className="text-slate-500">{userData.email}</p>
                  <p className="text-slate-500">
                    {userData.direccion} • {userData.telefono}
                  </p>
                </>
              )}
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 mt-2">
                ...
                <Badge className="bg-green-100 text-green-700 border-green-200">
                  ID: {userData.id ? userData.id.substring(0, 8) + "..." : ""}
                </Badge>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {userType === "donor" ? (
                <>
                  ...
                  <InfoItem label="Nombre completo" value={`${userData.nombre || ""} ${userData.apellido || ""}`} />
                  <InfoItem label="Correo electrónico" value={userData.email || ""} />
                  <InfoItem label="DNI" value={userData.dni || ""} />
                  <InfoItem label="Tipo de usuario" value="Donante" />
                </>
              ) : (
                <>
                  <InfoItem label="ID" value={userData.id || ""} />
                  <InfoItem label="Nombre" value={userData.nombre || ""} />
                  <InfoItem label="Correo electrónico" value={userData.email || ""} />
            ...
            {userType === "charity" && (
              <div className="mt-4 pt-4 border-t border-slate-100">
                ...
                <p className="text-slate-600">{userData.descripcion || "Sin descripción disponible."}</p>
              </div>
            )}
          </div>
        </CardContent>
