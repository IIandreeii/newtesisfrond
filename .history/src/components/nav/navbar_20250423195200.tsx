...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    // Función personalizada para el logout
    const customLogout = async () => {
      await postLogout();
      checkAuth(); // Verificar autenticación inmediatamente después del logout
    };

    // Sobrescribir la función postLogout
    const originalPostLogout = postLogout;
    // @ts-ignore - Ignorar error de TypeScript por reasignar postLogout
    postLogout = customLogout;

    return () => {
      clearInterval(interval);
      // @ts-ignore - Restaurar la función original
      postLogout = originalPostLogout;
    };
  }, [postLogout]);
  ...
  // Función de logout que actualiza el estado inmediatamente
